...
export function Project({ title, author, description, imageSrc }: { title: string; author: string; description: string; imageSrc?: string; }) {
    ...
    return <div className="bg-gray-50 out shadow-sm rounded-md">
        ...
        {imageSrc && <Image className="bg-background shadow-sm rounded-b-none rounded-t-sm" style={{ aspectRatio: "auto 4/3" }} src={imageSrc} alt={"Image"} width={720} height={720}></Image>}

        <div className="p-5">
            <h2 className="font-medium text-lg">{title}</h2>
            <p className="text-sm italic">- {author}</p>
            <br />
            {show ? <p>{description}</p> : <div>
