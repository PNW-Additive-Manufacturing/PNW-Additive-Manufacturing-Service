...
export function SupportedFilaments({filamentOptions, currentMaterials, setMaterialCallback, selectedFilament, setSelectedFilament}: {
  ...
  return (
    <>
      <select className="p-2 rounded" onChange={onChange} value={selectedFilament}>
        <option value="" defaultChecked={true}>--Select a Filament--</option>
        {filamentOptions.map(f => <option key={f} value={f}>{f.toUpperCase()}</option>)}
      </select>
      {currentMaterials.map(f => <input key={f} type="hidden" name="supported_materials" value={f}/>)}
      <div className="mt-2 p-2">
        {currentMaterials.map(f => <FilamentMaterialItem 
          key={f} 
          material={f} 
          onDelete={material => setMaterialCallback(currentMaterials.filter((f) => f !== material))}
        />)}
      </div>
    </>
  );
