...
export function PrinterForm({addPrinterCallback, filamentOptions, printerPreFill} : {addPrinterCallback: (p: Printer | null) => void, filamentOptions: string[], printerPreFill?: Printer}) {
  let [nameField, setNameField] = useState(printerPreFill?.name ?? "");
  let [modelField, setModelField] = useState(printerPreFill?.model ?? "");
  let [dimensionField1, setDimensionField1] = useState(printerPreFill?.dimensions[0] ?? "");
  let [dimensionField2, setDimensionField2] = useState(printerPreFill?.dimensions[1] ?? "");
  let [dimensionField3, setDimensionField3] = useState(printerPreFill?.dimensions[2] ?? "");

  let [materials, setMaterials] = useState(printerPreFill?.supportedMaterials ?? [] as string[]);
  let [selectedMaterial, setSelectedMaterial] = useState("");

  let [stratField, setStratField] = useState(printerPreFill?.communicationstrategy ?? "");
  let [optionsField, setOptionsField] = useState(printerPreFill?.communicationoptions ?? "");
  ...
  let addPrinterAction = async (prevState: string, formData: FormData) => {
    ...
    if(!printerPreFill) {
      ...
      result = await updatePrinter(prevState, formData, printerPreFill?.name);
    }
  ...
  return (
    <div>
      {printerPreFill ? <button className="float-right p-4 font-bold text-lg" onClick={(e) => addPrinterCallback(null)}>&times;</button> : <></>}
      <GenericFormServerAction 
        serverAction={addPrinterAction} 
        submitName={printerPreFill ? "Edit Printer" : "Add Printer" }
        submitPendingName={printerPreFill ? "Editing Printer..." : "Adding Printer..."}
      >
