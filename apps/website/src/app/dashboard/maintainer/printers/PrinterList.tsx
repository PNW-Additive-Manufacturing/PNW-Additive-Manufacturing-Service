...
export function PrinterList({initialPrinters, filamentMaterials} : {initialPrinters: Printer[], filamentMaterials: string[]}) {
  ...
  let doneEditingCallback = (printer: Printer | null) => {
    ...
    let newList = printers.slice();
    let editedPrinterIndex = newList.findIndex((p) => p.name === printer.name);
  ...
  return (
    <>
      {printerBeingEdited ? <EditPrinter doneEditingCallback={doneEditingCallback} filamentOptions={filamentMaterials} printer={printers.find((p => p.name === printerBeingEdited))!} ></EditPrinter> : <></>}
      <div>
            ...
            <tbody>
              {printers.map((p: Printer) => <tr id={p.name} key={p.name}>
                ...
                <td className='text-left pr-2'>
                  <button className='bg-red-500 p-1 rounded-lg border-none' onClick={(e) => deletePrinterClick(p.name)}>Delete</button>
                  <button className='bg-green-500 p-1 rounded-lg border-none' onClick={(e) => editPrinterClick(p.name)}>Edit</button>
                </td>
              </tr>)}
            </tbody>
          </table>
