...
export async function addPrinter(
	prevState: string,
	formData: FormData,
): Promise<{ error: string | null; printer: Printer | null }> {
	...
	let supportedMaterials: string[] = formData.getAll(
		"supported_materials",
	) as string[];
	let communicationStrat = formData.get("printer-communication") as string;
	let communicationStratOptions = formData.get(
		"printer-communication-options",
	) as string;

	try {
		let res =
			await db`insert into printer (name, model, dimensions, supportedmaterials, communicationstrategy, communicationstrategyoptions)
    values(${name},${model},${dimensions},${supportedMaterials},${communicationStrat},${communicationStratOptions}) returning name`;
		...
		return {
			error: "Failed to add printer with error: " + e.message,
			printer: null,
		};
	}
	...
	return {
		...
		printer: {
			...
			supportedMaterials,
			communicationoptions: communicationStratOptions,
		},
	};
}
...
export async function updatePrinter(
    ...
    `;
		...
		return {
			error: "Failed to update printer with error: " + e.message,
			printer: null,
		};
	}
...
export async function deletePrinter(
	...
	try {
		...
		let [row] =
			await db`select count(*) > 0 as printing from part where assignedprintername=${name} and status='printing'`;
